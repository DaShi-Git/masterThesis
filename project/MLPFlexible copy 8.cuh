...
// i saved this on 22.08, 这一版实现了flexble mm 多层， 在一个warp中, 保存用来对比32 batch size 和16 bs，这一版是8
//#include <vector>
...
__device__ int getCurrentWeightIndex(int layer){
  ...
  for (int i = 0; i < layer; i++){
    output = output + hiddenChannels[i]*hiddenChannels[i+1]*WMMA_M*WMMA_K;
  }
  return output;
}
...
__device__ int getCurrentBiasIndex(int layer){
  ...
  int output = 0;
  for (int i = 0; i < layer; i++){
    output = output + hiddenChannels[i+1]*WMMA_M;
  }
  return output;
}
...
int batch_size =  8; //batces in each warp
// Fragments
...
// wmma::fragment<wmma::matrix_a, 32, 8, WMMA_K, half, wmma::row_major>
//       test_frag;
//__syncthreads();
/////////////////////////////////////
...
int Cin16 = hiddenChannels[0]/WMMA_K;
int Cout16 = hiddenChannels[1]/WMMA_M; 

#pragma unroll
for(int i_d = 0; blockDim.x * i_d < Cin16*WMMA_K*batch_size * warpNum; ++i_d){
    int idx = blockDim.x * i_d + threadIdx.x;
//     if(threadIdx.x==0){
// printf("\n Cin %d\n", blockDim.x);}
    if(idx < Cin16*WMMA_K*batch_size * warpNum){
        d[idx] = __float2half(input[0][idx + loop * gridDim.x*Cin16*WMMA_K*batch_size * warpNum + blockIdx.x*warpNum*Cin16*WMMA_K*batch_size]);
    ...
    __syncthreads();
        ...
        #pragma unroll
				...
				{
					wmma::load_matrix_sync(b_frag[cin][0], warpID*Cin16*WMMA_K*batch_size + d + WMMA_K * cin, Cin16*WMMA_K);
...
Cin16 = hiddenChannels[hidden]/WMMA_K;
Cout16 = hiddenChannels[hidden+1]/WMMA_M;


  //load C to memory c(biass)
int i_c = 0;
#pragma unroll
while(blockDim.x * i_c < Cout16*WMMA_M){
  
    int idx = blockDim.x * i_c + threadIdx.x;
    if(idx < Cout16*WMMA_M){
      // printf("get in %d",  idx);
 ...
 #pragma unroll
  ...
  {
    wmma::load_matrix_sync(c_frag[cout][0], c + WMMA_M * cout, 0, wmma::mem_col_major);
...
//         }
 ...
 #pragma unroll
          ...
          #pragma unroll
					for (int cin = 0; cin < Cin16; ++cin){
						wmma::load_matrix_sync(a_frag[cout][cin],
							currentWeightIndex + a + WMMA_K * cin + Cin16*WMMA_K * WMMA_M * cout,
							Cin16*WMMA_K);
          }
        }
       //load C (bias)
...
#pragma unroll
        ...
        //activations
              ...
              c_frag[i][j].x[t] = activation[hidden](c_frag[i][j].x[t]);
        ...
        // //if (warpID==2){
        half* intermediateResults = d + batch_size * Cout16*WMMA_M * warpID;
        #pragma unroll
				for (int cout = 0; cout < Cout16; ++cout)
				{
					wmma::store_matrix_sync(intermediateResults + WMMA_M * cout, c_frag[cout][0], Cout16*WMMA_M, wmma::mem_col_major);
...
for(int i_output = 0; blockDim.x * i_output < batch_size*Cout16*WMMA_M * warpNum; ++i_output){//
    ...
    //if(idx < batch_size*Cout16*16 * warpNum){
       ...
       //printf("idx %d", idx + loop * gridDim.x*Cout16*16*batch_size * warpNum + blockIdx.x*warpNum*Cout16*16*batch_size);
        output[0][idx + loop * gridDim.x*Cout16*WMMA_M*batch_size * warpNum + blockIdx.x*warpNum*Cout16*WMMA_M*batch_size] = __half2float(d[idx]);
