...
// i saved this on 23.08, 这一版实现了flexble mm 多层， 在一个warp中, 之前遇到16个错数是因为循环多了sizeof（hiddenChannels）
...
for(int loop = 0; loop * gridDim.x*blockDim.x/32*batch_size < batchsizeTotal; ++i_output){//  loop around whole chip
...
//
//__syncwarp();
//printf("cout %d thread %d size of hidden cha%d\n", Cout16, threadIdx.x, sizeof(hiddenChannels)/sizeof(hiddenChannels[0]) -1);
} //end for hidden layers
__syncthreads();
//__syncwarp();

int loop = 0;
for(int i_output = 0; blockDim.x * i_output < batch_size*Cout16*16; ++i_output){//
    int idx = blockDim.x * i_output + threadIdx.x;
    if(idx < batch_size*Cout16*16){
       //output[0][idx] = __half2float(d[idx]);
       //output[0][idx] = __half2float((half(8.0)));
       //printf("thread %d\n", Cout16*16);
       
        output[0][idx + loop*gridDim.x*blockDim.x + blockDim.x*blockIdx.x] = __half2float(d[idx]);
    }
}
//
} //end of loop grid
...
if (warpID==0 && threadIdx.x == 0){
      ...
      for (int i = 0; i < batch_size*Cout16*16; ++i){
