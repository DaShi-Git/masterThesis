...
// i saved this on 20.08, to evaluate the bank conflict of half precision.
...
__shared__	half d[32 * 32*8 *3];
    ...
    // __shared__	half d[32 * 32]; // here need to be changed
    // __shared__	half a[32 * 32];
    // //__shared__	half b[K_GLOBAL1 * N_GLOBAL1*6];
   ...
   for(int i=0; i<16*16;++i){
    ...
    //   test_frag[10][10];
      wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, half> test_frag[10][10];
for (int i1 = 0; i1 < 1000000; ++i1) {
          //#pragma unroll
					for (int j1 = 0; j1 < 100; ++j1) {
           //#pragma unroll
						//for (int k = 0; k < Cin16; ++k) {
							//wmma::load_matrix_sync(test_frag[i1][j1], a1, 16, wmma::mem_col_major);
              a1[blockIdx.x] = a1[blockIdx.x]+b1[blockIdx.x];
              //b1[blockIdx.x] = a1[blockIdx.x]+b1[blockIdx.x];
              //__syncthreads();
						//}
					}
				}
      for(int l=0; l<1; l++){
        ...
        //wmma::fill_fragment(test_frag, (half)0.0f);
        // for (int i = 0; i < 10; ++i) {
				// 	for (int j = 0; j < 10; ++j) {
				// 		for (int t = 0; t < test_frag[0][0].num_elements; t++)
				// 		{
        //       //half tmp = (half)1;//c_frag[i][j].x[t];
        //       // tmp = tmp + (half)5;
				// 			// c_frag[i][j].x[t] = tmp;
        //       test_frag[i][j].x[t] = (half)0;//c_frag[i][j].x[t];
      ...
      // wmma::load_matrix_sync(test_frag, b1, 16);


  int currentWeightIndex = 0; //getCurrentWeightIndex(hidden, batchsize, featuresize);
  ...
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half, wmma::col_major>
      b_frag[10][2];
wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, half> c_frag[10][2];
  static constexpr int Cin16 = 8;
static constexpr int Cout16 = 8;  
// for (int cout = 0; cout < Cout16; ++cout)
// 				{
// 					wmma::fill_fragment(c_frag[cout][0], const half (half)0.0f);
// 					wmma::fill_fragment(c_frag[cout][1], const half (half)0.0f);
// 				}
    //for (int hidden = 0; hidden < 1; ++hidden){
// half* ptrshbias = shbias;
// wmma::load_matrix_sync(new_frag[0][0], ptrshbias, 16, wmma::mem_col_major);
//wmma::load_matrix_sync(c_frag[cin][0], c + 16 * cin, Cin16*16, wmma::mem_col_major);

    
    const int M_GLOBAL = Cout16*16;
    const int N_GLOBAL = 32;
    const int K_GLOBAL = Cin16*16;
 ...
 if (((N_GLOBAL*K_GLOBAL) % BLOCK_SIZE)>0){
  ...
  for (int i = 0; i < numloop_d; i++){
    if (i * BLOCK_SIZE + threadIdx.x < N_GLOBAL*K_GLOBAL){ 
    c[i * BLOCK_SIZE + threadIdx.x] = __float2half(bias[i * BLOCK_SIZE + threadIdx.x][0]);
    }
...
for (int hidden = 0; hidden <0; ++hidden){
...
//#pragma unroll
				...
				{
					wmma::load_matrix_sync(c_frag[cin][0], c + 16 * cin, Cin16*16, wmma::mem_col_major);
					wmma::load_matrix_sync(c_frag[cin][1], c + 16 * cin + 16 * Cin16*16, Cin16*16, wmma::mem_col_major);
				}
...
// half test1 = (half)2.0;
// half test2 = __shfl_sync(0xffffffff, test1, 0);
// printf("test2 is %f", __half2float(test2));
...
for (int p = 0; p < 32; p++)
  ...
  if (threadIdx.x == 32*warpID+p){
      ...
      for (int t = 0; t < 8; t++)
              ...
              //if (test_frag[0][0].x[t] == (half)0){
                //if (t < 8){
							//printf("thread[%d] frag_b.x[%d] is %.3f \n",p,t, __half2float(c_frag[3][0].x[t]));
