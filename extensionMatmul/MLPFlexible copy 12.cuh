...
// i saved this on 21.08, 这一版实现了准确mm，灵活改变参数. 发现了之前版本的问题，cfrag load and  store 都要用Cout16， c[]没定义前不要用 
...
while(blockDim.x * i_a < Cin16*16*Cout16*16){
    ...
    if(idx < Cin16*16*Cout16*16){
      // printf("get in %d",  idx);
        a[idx] = __float2half(weights[0][idx]);//
    }
    i_a += 1;
}
 //load A (weights)
