...
// __restrict__	half d[32 * 32 *8];
// __restrict__	half a[32 * 32 *8];
// __restrict__	half c[32 * 32 *8];

// __global__	__device__	half d[32 * 32 *8];
// __global__	__device__	half a[32 * 32 *8];
// __global__	__device__	half c[32 * 32 *8];

//__shared__ float sD[M_GLOBAL * N_GLOBAL*12];
__global__ void EvaluateMLPFlexible(
  kernel::Tensor2RW<real_t> weights, kernel::Tensor2RW<real_t> input, kernel::Tensor2RW<real_t> output, half *a1, half *d1, half *c1, float *dd1, int m_ld,
    ...
    // __restrict__	half d[M_GLOBAL1 * N_GLOBAL1 *8]; // here need to be changed
    // __restrict__	half a[M_GLOBAL1 * K_GLOBAL1*8];
    // //__shared__	half b[K_GLOBAL1 * N_GLOBAL1*6];
    // __restrict__	half c[M_GLOBAL1 * N_GLOBAL1*8];
  ...
  for (int i = 0; i < numloop; i++){
    a[i * 512 + threadIdx.y  * 128 + threadIdx.x] = __float2half(weights[i * 512 + threadIdx.y  * 128 + threadIdx.x][0]);
  ...
  //int numloop = 2;
  for (int i = 0; i < numloop; i++){
    d[i * 512 + threadIdx.y  * 128 + threadIdx.x] = __float2half(input[i * 512 + threadIdx.y  * 128 + threadIdx.x][0]);
...
//   int numloop = 2;
//   for (int i = 0; i < numloop; i++){
//     c[i * 512 + threadIdx.y  * 128 + threadIdx.x] = __float2half(weights[currentWeightIndex+i * 512 + threadIdx.y  * 128 + threadIdx.x][0]);
...
//__syncthreads();
  ...
  if (warpM==0&&warpN==0 && threadIdx.x == 0){
    for (int i = 0; i < 16*16*2*2; ++i){
      d[i] = sine_act(d[i])*__float2half(20);
      // printf("d[%d], %.3f ", i, __half2float(d[i]));
      // printf("sined[%d], %.3f ", i, __half2float(sine_act(d[i])));
    }
  }
// if (warpM==0&&warpN==0 && threadIdx.x == 0){
  ...
  // __syncthreads();
    ...
    if (warpM==0&&warpN==0 && threadIdx.x == 0){
      for (int i = 0; i < 100; i++) {
        ...
        //weights[0][0] = input[0][0];
          //t1 = weights[0][0];
          //weights[0][0]= i/j;
          //d[0] = a[0];
          //d[0] = __float2half(i/j);
          //output[0][0] = d[0];
          //d[0]= output[0][0];
