...
torch::Tensor evaluate_static_MLP(
	torch::Tensor& positions, const torch::Tensor& direction, const torch::Tensor& bias)
{
  CUstream stream = c10::cuda::getCurrentCUDAStream();
	...
	const std::string kernelName = "EvaluateMLPStatic";
	...
	extraSource << "#include \"MLPStatic.cuh\"\n";
...
// 	((void)ctx);
  ...
  m.def("evaluate_static_MLP", &evaluate_static_MLP, "Static MLP evaluate (CUDA)");
